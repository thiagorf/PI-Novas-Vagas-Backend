...
export class GetJobsForApplicantUseCase {
    constructor(private applicantRepository: ApplicantRepository, private applyRepository: ApplyRepository) {}
    ...
    async perform(applicant_id: number, authenticated_id: number) {
        ...
        const applicantJobs = await this.applicantRepository.getApplicantJobs(authenticated_id);
